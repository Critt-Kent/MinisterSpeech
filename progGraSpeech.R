...
ProgGraph <- function(X1=0, X2=0, Y1=0, Y2=0, fix=1, pu=0, fu=0, au=0, seg=1, CK=0, label=1, stime=0) {
...
#cat( "KEY", i, length(Keys$Time), "X1:", X1, "X2:", X2, "\n");
    ...
    # STime for interpreting data
    if(stime == 1) {
      stime = 0;
      for (i in 1:length(Source$STime)) {
        if(Source$STime[i] == 0) { next;}
        if(Source$Id[i] < Y1 || Source$Id[i] > Y2) {next;}
        if(X2 == X1){ X1 <- Source$STime[i];X2 <- Source$STime[i]+1; next;}
        if(X1 > Source$STime[i]) {X1 <- Source$STime[i];}
        if(X2 < Source$STime[i]) {X2 <- Source$STime[i];}
        stime <- 1;
    } }
  }
...
# Mit Zahlen an der Y-achse
  ...
  # print STime 
  if(stime) {
    text(Source$STime + 10 + (Source$Sdur/2),Source$Id, as.character("o"), cex=1.0, col="orange");

  ## Polygon STime Units
    for (i in 1:length(Source$STime)) { 
      if(Source$STime[i] > X2) {break;}
      if(Source$STime[i]+Source$Sdur[i] < X1) {next}
      
      min <- Source$Id[i]-0.5;
      max <- Source$Id[i]+0.5;
      
      rect(Source$STime[i], min, Source$STime[i]+Source$Sdur[i], max, border="orange", col=NA);
...
overlayTrack <- function(df, time="Time1", dur="Dur", char=".", color="blue", X1=0, X2=0) {
  
#  if(X2==0) {X2=length(df[,2])}
  
  dfT = c(t(df[time]))
  dfD = c(t(df[dur]))
  text(dfT + 10 + dfD/2, df$Id, char, cex=1.0, col=color);
  
  ## Polygon STime Units
  for (i in 1:length(dfT)) { 
    if(X2 > 0 && dfT[i] > X2) {break;}
    if(dfT[i] + dfD[i] < X1) {next}
    
    min <- df$Id[i]-0.5;
    max <- df$Id[i]+0.5;
    
    rect(dfT[i], min, dfT[i]+dfD[i], max, border=color, col=NA);
